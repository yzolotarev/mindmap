				...
				return {
					message: {
						...
						content: "**MINDMAP — Результат**\n\nСоединения юзера:\n```\n" + exp + "\n```\n\n" +
							"ЗАЧЕМ: карта совмещает две ментальные модели — юзера и твою. Понимает тот, кто рисует " +
							"стрелки. Правка = ВЫЗОВ, не приговор: юзер вправе защищать свою гипотезу, первая карта " +
							"обязана быть кривой. Ищи ИСКАЖЕНИЯ (систематические ошибки в том, КАК он связывает), " +
							"не только дыры. Тупик (всё входит, ничего не выходит) и остров (кучка без связей) — " +
							"не правка, а вопрос «и что с того? на что влияет?». " +
							"Когда консенсус достигнут — выведи «Договор по карте» (3-5 строк связей словами) и " +
							"ОБЯЗАТЕЛЬНО сверяй с ним дальнейшую работу; отклоняешься — скажи явно. " +
							"Объекты, которые юзер пометил «?» (СКМ по объекту на канвасе) — переформулируй их проще " +
							"на следующей итерации (аналогия из знакомой юзеру области + где она ломается), " +
							"это сигнал непонятного слова, не ошибка связи. " +
