			...
			if (exp) {
				...
				return {
					message: {
						...
						content: "**MINDMAP — Результат**\n\nИсходные:\n```\n" + scene + "\n```\n\nСоединения юзера (`->`/`--` обычная, `=>`/`==` жирная=главная, `: текст` = метка связи; блок POSITIONS даёт zone=top/mid/bottom-left/mid/right для каждого узла — реальное расположение в окне канваса):\n```\n" + exp + "\n```\n\n" +
							"Разбери как учитель по методу Санга. Ответ — ВСЕГДА картинка, никогда не список стрелок " +
							"текстом («картина стоит тысячи слов»): разложи боксы ┌─┐/│ │/└─┘ по ТОЙ ЖЕ 3x3 сетке, что в " +
							"POSITIONS (pattern recognition — юзер должен узнать своё расположение), пустые зоны пропусти. " +
							"Рёбра между непримыкающими клетками нечитаемы линией в моноширинном тексте — пиши их как " +
							"аннотацию ПОД боксом-источником: глиф →↓↑←↗↘↙↖ + буква цели + короткая подпись, " +
							"исправления — надписи ПРЯМО в этой строке (не отдельным текстом после):\n" +
