				...
				return {
					message: {
						...
						content: "**MINDMAP — Результат**\n\nСоединения юзера:\n```\n" + exp + "\n```\n\n" +
							...
							"Запусти: python3 ~/bin/mindmap-ascii.py --mark \"X-Y:add,X-Y:wrong\" (макс 2 записи в --mark, " +
							"остальное — в следующую итерацию). Он рисует НАСТОЯЩИЕ линии по сохранённым координатам юзера. " +
							"add = не хватало стрелки (пунктир, метка +), wrong = его стрелка неверна (метка x на конце). " +
							"Вставь вывод как есть, без пересказа. Каждую правку — ОДНОЙ бытовой фразой вида " +
							...
							"Под картинкой — максимум две такие фразы + один вопрос «почему?» про ключевую стрелку юзера. В конце (если тема закрыта) предложи: углубить группу (Layer 2) или " +
