...
"""
import locale
# Без выставленной локали Tk на Linux не переводит кириллические keysym в
# символы (латиница вводится, кириллица — нет). Ставим до создания Tk root,
# чтобы диалоги создания/переименования нод принимали русский ввод.
for _loc in ("", "C.UTF-8", "en_US.UTF-8", "ru_RU.UTF-8"):
    try:
...
# Fallback для кириллицы: когда XIM/локаль не переводит keysym в символ
# (event.char пустой), вставляем букву сами по имени keysym. Локали недостаточно
# на части систем — проверено вживую 10.07.26.
_CYR_LOWER = {
...
CYR_KEYSYMS = dict(_CYR_LOWER)
for _ks, _ch in _CYR_LOWER.items():
    CYR_KEYSYMS["Cyrillic_" + _ks.split("_", 1)[1].upper()] = _ch.upper()

def _cyr_key(event):
    ch = CYR_KEYSYMS.get(event.keysym)
    if ch:
        event.widget.insert(tk.INSERT, ch)
        return "break"  # не дать Tk вставить свой (пустой/кривой) вариант

def ask_string(root, title, prompt, initialvalue=""):
    """Замена simpledialog.askstring с рабочим вводом кириллицы."""
    top = tk.Toplevel(root)
    top.title(title); top.configure(bg=BG)
    top.transient(root); top.grab_set(); top.resizable(False, False)
    tk.Label(top, text=prompt, bg=BG, fg=FG, font=("Helvetica", 10)).pack(padx=14, pady=(12, 4))
    ent = tk.Entry(top, bg=NODE_FILL, fg=FG, insertbackground=FG,
                   font=("Ubuntu", 11), width=34, relief=tk.FLAT)
    ent.insert(0, initialvalue)
    ent.pack(padx=14, pady=4, ipady=4)
    ent.bind("<KeyPress>", _cyr_key)
    ent.icursor(tk.END); ent.select_range(0, tk.END)
    res = {"v": None}
    def ok(e=None): res["v"] = ent.get(); top.destroy()
    def cancel(e=None): top.destroy()
    row = tk.Frame(top, bg=BG); row.pack(pady=(6, 12))
    for txt, cmd in (("OK", ok), ("Cancel", cancel)):
        tk.Button(row, text=txt, command=cmd, bg=BTN_BG, fg=BTN_FG,
                  relief=tk.FLAT, padx=14, pady=3, cursor="hand2").pack(side=tk.LEFT, padx=5)
    ent.bind("<Return>", ok); top.bind("<Escape>", cancel)
    ent.focus_set()
    top.wait_window()
    return res["v"]
...
class Connection:
    def __init__(self, ca, a, b, typ="arrow", label="", emph=False):
        ...
        self.id = self.label_id = self.label_bg_id = None
        self._draw()
    ...
    def _draw(self):
        ...
        if self.label:
            ...
            # плашка цвета фона под текстом, иначе подпись сливается с линией
            self.label_id = self.ca.create_text(
                mx, my-12, text=self.label, fill=c,
                font=("Ubuntu", 11, "bold"), tags="conn")
            bx1, by1, bx2, by2 = self.ca.bbox(self.label_id)
            self.label_bg_id = self.ca.create_rectangle(
                bx1-5, by1-2, bx2+5, by2+2, fill=BG, outline=c, width=1, tags="conn")
            self.ca.tag_raise(self.label_id, self.label_bg_id)
    ...
    def delete(self):
        ...
        if self.label_bg_id: self.ca.delete(self.label_bg_id); self.label_bg_id = None
...
class App:
    def __init__(self, root):
        ...
        self.redo_stack = []
        self.undo_limit = 50
        self._drag_snap = None    # снимок до перетаскивания (для undo движений)
    ...
    def _push_undo(self, snap):
        ...
        self.redo_stack.clear()  # новое действие обнуляет ветку redo
    ...
    def redo(self, e=None):
        if not self.redo_stack: return self._msg("Nothing to redo")
        self.undo_stack.append(self._capture())
        if len(self.undo_stack) > self.undo_limit: self.undo_stack.pop(0)
        self._restore(self.redo_stack.pop()); self._msg("Redone")
    ...
    def _build(self):
        ...
        self.status = tk.Label(self.root,
            text="C=arrow V=line B=group | LMB=drag RMB=connect 2xLMB(линия)=метка | Ctrl+Z/Ctrl+Y Ctrl+E",
    ...
    def _dlg_add(self):
        n = ask_string(self.root, "New Node", "Name:")
    ...
    def _lmb(self, e):
        ...
        if h:
            ...
            self._drag_snap = self._capture(); self._drag_moved = False
        else:
            for s in list(self.selected): s.select(False)
    ...
    def _drag(self, e):
        ...
        if self.drag and self.drag_xy:
            ...
            if dx or dy: self._drag_moved = True
        ...
        if self.rubber:
            ...
            if inside:
                ...
                if self.mode.get() == "group":
                    self._snap()
                    nm = ask_string(self.root, "Group", "Name (or Cancel):")
                    self.groups.append(GroupBox(self.ca, inside, nm.strip() if nm else None))
                    self._msg(f"Grouped {len(inside)} nodes")
            return
        if self._drag_moved and self._drag_snap:
            self._push_undo(self._drag_snap)
        self._drag_snap = None; self._drag_moved = False
        self.drag = None; self.drag_xy = None

    def _dbl(self, e):
        h = self._hit(e.x, e.y)
        if h and not getattr(h, 'is_group', False):
            nm = ask_string(self.root, "Rename", "New name:", initialvalue=h.name)
        ...
        if not h:
            for c in reversed(self.conns):
                if c.near(e.x, e.y):
                    ...
                    lb = ask_string(
                        self.root, "Связь",
                        "Метка связи ('!' в конце = жирная, пусто = убрать):",
                        initialvalue=init)
    ...
    def export(self):
        ...
        W = self.ca.winfo_width() or 1000
        H = self.ca.winfo_height() or 640
        def zone(px, py):
        ...
        for n in self.nodes:
            lines.append(f"  {idx[n]}: x={round(100*n.x/W)}% y={round(100*n.y/H)}% zone={zone(n.x,n.y)}")
        if self.conns:
            lines += ["", "=== CONNECTIONS ==="]
            for c in self.conns:
